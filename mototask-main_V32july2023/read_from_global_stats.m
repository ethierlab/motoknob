function [last_hit_thresh, last_median_peak, historical_HT_max] = read_from_global_stats(rat_id,folder,module)
%
       ...
       for i = 1:size(global_stats, 1)
            if strcmp(module, global_stats.device{i})
                ...
                if any(strcmp('Num_stimulations', global_stats.Properties.VariableNames))
                    num_stimulations = global_stats.Num_stimulations(i);
                end
                if any(strcmp('Historical_HT_max', global_stats.Properties.VariableNames))
                    historical_HT_max = global_stats.Historical_HT_max(i);
                end
            end
